...
define([ 'jquery', 'knockout', 'config/config', 'service', 'underscore', 'lib/jquery/chosen.jquery'],
    ...
    function ($, ko, config, service, _, chosen) {
        ...
        var templateColumns = {
            ...
            listColumns:[
                { columnType:"checkbox", headerTextTrans:"number", rowText:"index", width:"7%",textAlign:"center"},
                { headerTextTrans:"name", rowText:"name", width:"25%", sortable:true },
                { columnType:"image", headerTextTrans:"save_location", rowText:"imgLocation", width:"23%", sortable:true},
        ...
        function phoneBookVM() {
            ...
            self.booksText = ko.observableArray();
            ...
            function getNameMaxLength() {
                var max = 22;
                var encodeType = getEncodeType2(self.name());
                if (self.selectedLocation() == locationValue.DEVICE) {
                    if("ru" == encodeType.encodeType){
                        max = 23;
                    }else{
                        max = 26;
                    }
                    ...
                    //对"^"需要按照2个字符处理
                  if("ru" == encodeType.encodeType){
                      return self.capacity().simMaxNameLen - 3;
                  }
            ...
            self.checkSelect = function (pState) {
                var ids = [];
                if ("send" == pState) {
                    var checkedItem = $("#books :checkbox[checked='checked']");
            ...
            self.selectedAllBooks = function () {
                $("#books input").each(function(){
                    $(this).parent('p').addClass('checkbox_selected');
                    $(this).attr('checked','checked');
                })
                /*$("#pblist-checkall").trigger('click');*/
                if( $("#books input").length > 0){
                    $("#deselectLi").show();
                    $("#selectLi").hide();
                }

            };
            self.deselectedAllBooks = function () {
                $("#books input").each(function(){
                    $(this).parent('p').removeClass('checkbox_selected');
            ...
            self.openSendMessagePage = function () {
                ...
                select.append(opts);
                select.chosen({max_selected_options: 5, search_contains: true, width: '448px'});
            ...
            self.searchColumns = ['name','mobile_phone_number'];
            ...
            self.clearSearchKey = function () {
                self.searchInitStatus(true);
                self.searchKey($.i18n.prop("search"));
            ...
            self.searchTextClick = function () {
                ...
                if (searchText.hasClass("ko-grid-search-txt-default")) {
                    self.searchKey("");
                    self.searchInitStatus(false);
                    searchText.removeClass("ko-grid-search-txt-default").removeAttr("trans");
                }
            };
            self.filterGroup = ko.observable('all');
            self.filterPhoneByGroup = function(item){
               var groupsArray =  $("#groupLi li a");
                for(var i = 0 ;i < groupsArray.length ; i++){
                    if( $(groupsArray[i]).attr("trans").indexOf(item) != -1){
                        $(groupsArray[i]).addClass('cus_hover');
                    }else{
                        $(groupsArray[i]).removeClass('cus_hover');
                    }
                }

                self.filterGroup(item);
            }
            self.afterSearchData = ko.computed(function () {
                ...
                if (key == "" && self.filterGroup() == "all") {
                    return self.booksText();
                }else {
                    return _.filter(self.booksText(), function (item) {
                        if(self.filterGroup() !='all'&& item["group"] != self.filterGroup()){
                            return false;
                        }
            ...
            self.searchTextBlur = function () {
                var txt = $.trim(self.searchKey()).toLowerCase();
                if (txt == "") {
                    self.clearSearchKey();
                }
            };
            ...
            function getPhoneBookReady() {
                service.getPhoneBookReady({}, function (data) {
                    if (data.pbm_init_flag == "6") {
                        ...
                        var phoneBooks2 = getBooks2(capacity.Used);
                        self.books(phoneBooks);
                        self.booksText(phoneBooks2);
            ...
            function setPreContent() {
                ...
                $("#backText").removeClass("cus1ForBut");
            }
        ...
        function getBooks2(capacity) {
            ...
            return translateData2(books.pbm_data);
        }
        function getBooksData(capacity) {
            var para = {};
            para.page = 0;
            para.data_per_page = capacity;
            para.orderBy = "name";
        ...
        function init() {
            ...
            $("#txtName,#txtMobile,#txtSmsContent").die().live("drop", function(){
                 return false;
